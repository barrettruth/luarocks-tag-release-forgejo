describe('Action entrypoint', function()
  local function run_action(env_vars)
    local command = 'env -u LUAROCKS_API_KEY -u GITHUB_ACTION_PATH -u GITHUB_EVENT_NAME LUA_PATH="lua/?.lua;;" '
  ...
  it('does not require the LuaRocks API key for pull request runs', function()
    local output = run_action { 'GITHUB_EVENT_NAME=pull_request' }

    assert.matches('GITHUB_ACTION_PATH not set.', output, 1, true)
